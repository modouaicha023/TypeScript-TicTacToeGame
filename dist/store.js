...
class Store extends EventTarget {
    ...
    get game() {
        ...
        for (const player of this.players) {
            const selectedSquaredId = state.currentGameMoves.filter((move) => move.player.id === player.id).map(move => move.squareId);
...
_Store_instances = new WeakSet(), _Store_getState = function _Store_getState() {
    ...
    window.localStorage.setItem(this.storageKey, JSON.stringify(newSate));
